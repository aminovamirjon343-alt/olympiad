...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-2xl font-bold mb-6">Редактировать пользователя: {{ $user->name }}</h1>

    @if ($errors->any())
    {{-- ⚠️ ИСПРАВЛЕНИЕ: Опечатка border-red-red-400 заменена на border-red-400 --}}
    ...
    <form action="{{ route('superadmin.users.update', $user->id) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            <label for="password" class="block text-gray-700 text-sm font-bold mb-2">Новый пароль (оставьте пустым, если не хотите менять):</label>
        ...
        <div class="mb-4 flex items-center">
            ...
            <input type="checkbox" name="is_admin" id="is_admin" value="1" class="mr-2 leading-tight" {{ old('is_admin', $user->is_admin) ? 'checked' : '' }}>
        ...
        <div class="mb-6 flex items-center">
            ...
            <input type="checkbox" name="is_super_admin" id="is_super_admin" value="1" class="mr-2 leading-tight" {{ old('is_super_admin', $user->is_super_admin) ? 'checked' : '' }}>
