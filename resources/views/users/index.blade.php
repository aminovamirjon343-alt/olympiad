...
@section('content')
                                        ...
                                        <td class="text-right py-2 px-3">
                                            <div class="flex items-center justify-end gap-1">
                                                @php
                                                    ...
                                                    // 1. Кто я? (Должен быть админ или директор)
                                                    $amIAdmin = in_array($myRole, ['admin', 'админ', 'director']);

                                                    // 2. Супер-админ (ему можно всё, кроме удаления себя)
                                                    $isSuperAdmin = ($authUser->email === 'aaamirtrending14@gmail.com');
                                                    ...
                                                    // 5. Можно ли трогать эту роль? (USER - неприкасаемые)
                                                    $isNotUserRole = ($targetUserRole !== 'user');

                                                    // --- ФИКС ЛОГИКИ ---
                                                    // Кнопка изменить:
                                                    // Ты админ И (это ты сам ИЛИ ты его создал ИЛИ ты супер-админ) И это не роль 'user'
                                                    $canManage = $amIAdmin && ($isMe || $iCreatedHim || $isSuperAdmin) && $isNotUserRole;

                                                    // Кнопка удалить:
                                                    // Все то же самое, но себя удалять нельзя
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const translations = {
                ru: { pageTitle: "Пользователи", pageSubtitle: "Управление командой и доступом", addBtn: "Добавить", thEmployee: "Сотрудник", thEmail: "Email / Контакты", thPhone: "Телефон", thRole: "Роль", thActions: "Управление", statusOnline: "Онлайн", statusOffline: "Офлайн", confirmDelete: "Удалить пользователя?" },
                tj: { pageTitle: "Корбарон", pageSubtitle: "Идоракунии даста ва дастрасӣ", addBtn: "Илова кардан", thEmployee: "Корманд", thEmail: "Email / Тамос", thPhone: "Телефон", thRole: "Нақш", thActions: "Идоракунӣ", statusOnline: "Онлайн", statusOffline: "Офлайн", confirmDelete: "Корбарро нест мекунед?" }
            };
