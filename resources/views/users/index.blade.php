...
@section('content')
    <div class="min-h-screen bg-[#f1f5f9] py-8">
        <div class="container mx-auto px-4">
            <style>
                @import url('https://fonts.googleapis.com/css2?family=Inter+Tight:wght@400;700;900&family=Inter:wght@400;600;800&display=swap');
                .users-page { font-family: 'Inter', sans-serif !important; }
                .users-page h1 { font-family: 'Inter Tight', sans-serif !important; }
                .table-card { background: #f8fafc !important; border-radius: 1.25rem; border: 1px solid rgba(15, 23, 42, 0.08); box-shadow: inset 0 2px 4px rgba(0,0,0,0.02); overflow: hidden; }
                .table-header-primary { background: linear-gradient(90deg, var(--primary, #2563eb) 0%, #3b82f6 100%); }
                .users-table td { padding: 0.85rem 1.25rem; font-size: 0.8rem; color: #334155 !important; vertical-align: middle; border-bottom: 1px solid #f1f5f9; }
                .tr-hover:hover { background-color: #ffffff !important; transition: background 0.2s ease; }
                .btn-primary-system { background-color: var(--primary, #4f46e5) !important; transition: all 0.2s ease; }
            </style>
                    ...
                    <div class="table-card overflow-hidden">
                        <div class="overflow-x-auto">
                            <table class="w-full text-left users-table border-collapse">
                                <thead class="table-header-primary">
                                <tr>
                                    <th class="w-12 text-center py-2 px-3 text-[10px] font-medium text-white/80 tracking-wide" data-i18n="id">Id</th>
                                    <th class="py-2 px-3 text-[10px] font-semibold text-white/90 tracking-wide" data-i18n="employee">Сотрудник</th>
                                    <th class="py-2 px-3 text-[10px] font-normal text-white/70 tracking-wide" data-i18n="contacts">Email / контакты</th>
                                    <th class="py-2 px-3 text-[10px] font-normal text-white/70 tracking-wide" data-i18n="phone">Телефон</th>
                                    <th class="text-center py-2 px-3 text-[10px] font-normal text-white/70 tracking-wide" data-i18n="role">Роль</th>
                                    <th class="text-right py-2 px-3 text-[10px] font-normal text-white/70 tracking-wide" data-i18n="manage">Управление</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($users as $user)
                                    <tr class="group tr-hover border-b border-slate-50 last:border-0 transition-colors duration-150">
                                        <td class="text-center font-black text-slate-300 text-[9px] py-2 px-3">#{{ str_pad($user->id, 3, '0', STR_PAD_LEFT) }}</td>
                                        <td class="py-2 px-3">
                                            <div class="flex items-center gap-2">
                                                <div class="relative">
                                                    @php
                                                        $colors = [['bg' => '#e0f2fe', 'text' => '#0369a1'], ['bg' => '#dcfce7', 'text' => '#15803d'], ['bg' => '#fef3c7', 'text' => '#b45309'], ['bg' => '#f3e8ff', 'text' => '#7e22ce'], ['bg' => '#fee2e2', 'text' => '#b91c1c']];
                                                        $currentColor = $colors[$user->id % 5];
                                                    @endphp
                                                    <div class="w-6 h-6 rounded flex items-center justify-center text-[9px] font-black shadow-sm border border-black/5" style="background-color: {{ $currentColor['bg'] }}; color: {{ $currentColor['text'] }};">
                                                        {{ Str::upper(Str::substr($user->name, 0, 1)) }}
                                                    </div>
                                                    @if($user->isOnline())
                                                        <span class="absolute -bottom-0.5 -right-0.5 flex h-2.5 w-2.5"><span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span><span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-green-500 border-2 border-white"></span></span>
                                                    @else
                                                        <span class="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-slate-300 border-2 border-white"></span>
                                                    @endif
                                                </div>
                                                <div class="flex flex-col leading-tight">
                                                    <div class="font-bold text-slate-800 text-[13px] leading-none group-hover:text-blue-600">{{ $user->name }}</div>
                                                    <span class="mt-1 text-[9px] font-medium tracking-wide {{ $user->isOnline() ? 'text-green-600' : 'text-slate-400' }}" data-i18n="{{ $user->isOnline() ? 'statusOnline' : 'statusOffline' }}"></span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="font-medium text-slate-500 text-[10px] py-2 px-3">{{ $user->email }}</td>
                                        <td class="py-2 px-3"><span class="font-black text-slate-700 text-[9px]">{{ $user->phone ?? '00 000 0000' }}</span></td>
                                        <td class="text-center py-2 px-3">
                                            @php
                                                $roleKey = match(strtolower(trim($user->role))) {
                                                    'admin', 'админ' => 'roleAdmin',
                                                    'director', 'директор' => 'roleDirector',
                                                    'employee', 'сотрудник' => 'roleEmployee',
                                                    default => 'roleUser'
                                                };
                                            @endphp
                                            <span class="role-badge inline-block px-2 py-0.5 rounded text-[8px] font-black uppercase border shadow-sm {{ $roleKey === 'roleAdmin' ? 'text-white' : 'bg-white text-slate-500 border-slate-200' }}"
                                                  style="{{ $roleKey === 'roleAdmin' ? 'background-color: var(--primary); border-color: var(--primary);' : '' }}"
                                                  data-role-key="{{ $roleKey }}"></span>
                                        </td>
                                        <td class="text-right py-2 px-3">
                                            <div class="flex items-center justify-end gap-1">
                                                <a href="{{ route('users.show', $user->id) }}" class="p-1.5 bg-white border border-slate-100 text-slate-400 hover:text-blue-600 rounded-md shadow-sm"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                                @if(auth()->id() === 10 || auth()->id() === (int)$user->id || (strtolower(auth()->user()->role) === 'admin' && (int)$user->created_by === auth()->id()))
                                                    <a href="{{ route('users.edit', $user->id) }}" class="p-1.5 bg-white border border-slate-100 text-slate-400 hover:text-amber-500 rounded-md shadow-sm"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg></a>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const t = {
                ru: { pageTitle: "Пользователи", pageSubtitle: "Управление командой и доступом", addBtn: "Добавить", id: "Id", employee: "Сотрудник", contacts: "Email / контакты", phone: "Телефон", role: "Роль", manage: "Управление", statusOnline: "Онлайн", statusOffline: "Офлайн", roleAdmin: "Админ", roleDirector: "Директор", roleEmployee: "Сотрудник", roleUser: "Пользователь" },
                tj: { pageTitle: "Корбарон", pageSubtitle: "Идоракунии даста ва дастрасӣ", addBtn: "Илова кардан", id: "Id", employee: "Корманд", contacts: "Email / тамос", phone: "Телефон", role: "Нақш", manage: "Идоракунӣ", statusOnline: "Онлайн", statusOffline: "Офлайн", roleAdmin: "Админ", roleDirector: "Директор", roleEmployee: "Корманд", roleUser: "Корбар" },
                en: { pageTitle: "Users", pageSubtitle: "Team and access management", addBtn: "Add New", id: "Id", employee: "Employee", contacts: "Email / contacts", phone: "Phone", role: "Role", manage: "Manage", statusOnline: "Online", statusOffline: "Offline", roleAdmin: "Admin", roleDirector: "Director", roleEmployee: "Employee", roleUser: "User" }
            };
            const lang = localStorage.getItem('app-lang') || 'ru';
            ...
            document.querySelectorAll('[data-role-key]').forEach(el => { if(dict[el.getAttribute('data-role-key')]) el.textContent = dict[el.getAttribute('data-role-key')]; });
        });
    </script>
@endsection
